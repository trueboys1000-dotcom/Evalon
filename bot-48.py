"""
╔══════════════════════════════════════════════════════════════╗
║         EVALON WINNERS — TELEGRAM SUPPORT BOT v6.6          ║
║                                                              ║
║  ✅ Multi-step menu                                          ║
...
def svc_keyboard(lang, indicator=False, signals=False, autobot=False, social=False):
    ...
    if signals:
        rows.append([
            InlineKeyboardButton("▶️ Watch Signal Results", url="https://youtu.be/rflkvCWG-fw?si=VFWoZO1cPqCQbg83"),
            InlineKeyboardButton("📱 TikTok", url="https://vt.tiktok.com/ZSx2do7SF/"),
        ])
        rows.append([
            InlineKeyboardButton("📱 More Results", url="https://vt.tiktok.com/ZSx2d3RmU/"),
        ])

    if autobot:
        rows.append([
            InlineKeyboardButton("▶️ Watch Bot Results", url="https://youtu.be/q3Sa9ndExNc?si=MmMVL9F7t-fRzXQF"),
            InlineKeyboardButton("📱 TikTok", url="https://vt.tiktok.com/ZSx2RHF25/"),
        ])

    if social:
        rows.append([
            InlineKeyboardButton("▶️ Watch Social Results", url="https://vt.tiktok.com/ZSx2RNM2m/"),
            InlineKeyboardButton("📱 More", url="https://vt.tiktok.com/ZSx2dpgxt/"),
        ])
    ...
    if indicator:
        ...
        rows.append([
            InlineKeyboardButton("▶️ Watch Demo", url="https://vt.tiktok.com/ZSx2dGWab/"),
            InlineKeyboardButton("📱 TikTok 2", url="https://vt.tiktok.com/ZSx2RQcpD/"),
        ])
        rows.append([
            InlineKeyboardButton("📱 TikTok 3", url="https://vt.tiktok.com/ZSx2RrX7T/"),
        ])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        try:
            ...
            # Fallback — send plain text without parse mode
            try:
                msg = await context.bot.send_message(
                    chat_id=cid,
                    text=f"🎁 Your referral link:\nhttps://t.me/{BOT_USERNAME}?start=ref{user.id}\n\nReferrals: {ref_count}/{REFERRAL_MIN}\nInvite {needed} more to unlock reward!",
                    protect_content=True,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(ui("btn_back", lang), callback_data="main_menu")],
                    ]))
    ...
    # ── SPIN WHEEL ─────────────────────────────────────────────
        ...
        if not can_spin_today(user.id):
            hours, mins = get_next_spin_time()
            await query.answer(
                ui("spin_wait", lang).format(hours=hours, mins=mins),
                show_alert=True)
            return

        # Record spin immediately to prevent double-spin
        record_spin(user.id)
        ...
        result_header = (
            f"🎰 *LUCKY SPIN RESULT* 🎰\n\n"
            f"{'🎉 *YOU WON!* 🎉' if is_win else '💫 *SPIN RESULT*'}\n\n"
            f"{prize_text}"
        )
        ...
        # Notify admin if user won
        if is_win:
            name = escape_md(user.full_name)
            username = escape_md(user.username or "NA")
            prize_display = {
                "private_session": "🎯 FREE Private Support Session",
                "free_bot":        "🤖 FREE Manual Bot",
                "free_signal":     "📊 FREE VIP Signal",
            }.get(prize_key, prize_key)
            for aid in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=aid,
                        text=(
                            ...
                            f"⚠️ Please fulfill this prize manually!"
                        ),
                        parse_mode="Markdown",
                        reply_markup=InlineKeyboardMarkup([[
                            InlineKeyboardButton(
                                "🟢 Connect to Deliver", callback_data=f"con:{user.id}:{lang}")
                        ]]))
                except:
                    pass
...
def main():
    ...
    print(f"✅ {BUSINESS_NAME} Bot v6.5 is LIVE!")
    print("📋 Commands: /broadcast  /stats  /getid  /sessions")
    print("🎰 Spin Wheel: 1x/day, 5% win chance")
    app.run_polling(
