...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── MY DAILY STREAK ────────────────────────────────────────
        ...
        # Show spinning animation — SHORT: 5 frames, 3 seconds total
        ...
        # Only 5 frames, faster timing to avoid Telegram rate limit & timeout
        spin_timings = [0.5, 0.5, 0.6, 0.7, 0.8]
        for i, (frame, wait) in enumerate(zip(SPIN_FRAMES[:5], spin_timings)):
            ...
            try:
                ...
                pass  # message already edited or deleted — continue anyway
            except Exception as e:
                if "retry" in str(e).lower() or "flood" in str(e).lower():
                    await asyncio.sleep(3)
                # Always continue — never break out early
...
async def setvip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Only saves if there IS existing content AND it hasn't been saved today already
    ...
    if old_vip and (old_vip.get("text") or old_vip.get("file_id")):
        ...
        # Check if this session was already saved today to avoid duplicates
        existing = get_results_history(5)
        already_saved_today = any(
            row[4] and row[4].startswith(today_str) and (old_vip.get("file_id") and row[2] == old_vip.get("file_id"))
            for row in existing
        )
