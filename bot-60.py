...
def get_daily_feedback(lang, count):
    """Get feedback for the day — rotates daily, same day same feedback"""
    pool = FAKE_FEEDBACK.get(lang, FAKE_FEEDBACK["en"])
    day_seed = datetime.now().timetuple().tm_yday + datetime.now().year
    rng = random.Random(day_seed)
    # Shuffle with today's seed
    shuffled = pool.copy()
    rng.shuffle(shuffled)
    # Return requested count (cycle if needed)
    result = []
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        # Main referral info — no link inside (sent separately below)
        ...
        # FIX: Send link as SEPARATE plain message — no markdown, no protect_content
        # This makes it easy to hold/tap and copy on mobile
        ref_link = f"https://t.me/{BOT_USERNAME}?start=ref{user.id}"
        try:
            link_msg = await context.bot.send_message(
                chat_id=cid,
                text=ref_link,
                protect_content=False)  # NOT protected so user can copy/share easily
    ...
    # ── MY DAILY STREAK ────────────────────────────────────────
        ...
        # Animate 20 frames — ~16 seconds total (0.8s each)
        for i, frame in enumerate(SPIN_FRAMES):
            await asyncio.sleep(0.8)
            try:
                # Slow down last 5 frames to simulate deceleration
                if i >= 15:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "`/feedback` — Preview 5 fake feedbacks (English)\n"
        "`/feedback sw 10` — Swahili, 10 feedbacks\n"
        "`/feedback ur 5` — Urdu feedbacks\n"
...
async def feedback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Send sequential fake feedback previews — changes daily automatically
    Usage: /feedback       → English (5 samples)
    Usage: /feedback sw    → Swahili (5 samples)
    Usage: /feedback ur    → Urdu (5 samples)
    Usage: /feedback en 10 → English (10 samples)
    """
    if not is_admin(update.effective_user.id):
        return
    ...
    if lang not in FAKE_FEEDBACK:
        lang = "en"

    total_available = {
        "en": len(FAKE_FEEDBACK["en"]),
        "sw": len(FAKE_FEEDBACK["sw"]),
        "ur": len(FAKE_FEEDBACK["ur"]),
    }

    await update.message.reply_text(
        f"📨 *FEEDBACK PREVIEW*\n\n"
        f"🌍 Language: `{lang}`\n"
        f"📊 Showing: {count} feedback\n"
        f"📁 Total available: {total_available.get(lang, len(FAKE_FEEDBACK['en']))}\n\n"
        f"_These rotate daily automatically — same day same feedback_\n"
        f"_Sending now one by one..._",
        parse_mode="Markdown")

    feedbacks = get_daily_feedback(lang, count)
    await asyncio.sleep(0.5)
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"📌 *Tips for recording:*\n"
            f"• Record your screen as messages appear\n"
            f"• Use `/feedback en 10` for more samples\n"
            f"• Use `/feedback sw 5` for Swahili\n"
            f"• Use `/feedback ur 5` for Urdu\n"
            f"• Messages change daily — fresh content every day!\n\n"
            f"🎬 After recording, use `/broadcast` to send the video to all users!"
        ),
        parse_mode="Markdown")
